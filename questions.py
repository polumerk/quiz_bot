...
def build_openai_prompt(theme: str, round_num: int, questions_per_round: int, history_text: str, difficulty: str = 'medium') -> str:
    # Добавим более конкретные примеры для спорта
    sport_examples = ""
...
- "Сколько очков дает гол в футболе?" → "1"
- "Как называется спорт с клюшками на льду?" → "Хоккей"

ИЗБЕГАЙ неточных вопросов типа:
- "Какой вид спорта проводится на поле с использованием клюшек и мячей?" (может быть хоккей с мячом, бенди, крикет, поло)
- "Какой вид спорта включает в себя бег с барьерами?" (может быть легкая атлетика, барьерный бег, стипльчез)"""
    
    return (
        START_PROMPT +
        f"\nТема: {theme}\nСгенерируй {questions_per_round} уникальных, точных и однозначных вопросов для раунда {round_num}. "
